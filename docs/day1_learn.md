...
# 总结将亡值到底是什么
好，这次我给你一个**“工程师速查版”的结构化总结**，按**概念 → 行为 → 内存 → 适用性 → 设计结论**来，每一块都配**最小例子**，你以后回来看这一条就够了。
...
```cpp
std::string a = "hello";
auto&& x = std::move(a);   // std::move(a) 是 xvalue
```
...
```cpp
std::string a;
std::string&& r = std::move(a); // 只是引用绑定
// a 完全没变
```
...
std::vector<int> v2 = std::move(v1); // O(1)
```
...
> move 从不移动对象的位置，只移动资源。

**例子：**

```cpp
std::string a = "hi"; // a 在栈上
...
> **C++ 移动语义的本质是“资源所有权转移”，
> 而不是对象内存位置的变化；
> std::move 只是把对象标记为“可被移动”，
> 是否真的移动，完全取决于类型是否支持。**

如果你愿意，我可以把这份总结**压成一张 A4 速查图**，或者帮你**用一段代码把所有情况跑一遍**。
